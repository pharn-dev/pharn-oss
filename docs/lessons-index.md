...
60 lessons · 60 tagged · 0 malformed · 0 untagged · ~43565 tokens total
...
L58 | floor    | temporal-state,referent-binding,append-only,snapshot-staleness,lesson-recurrence               | A record bound to a live referent must ask which part of that referent may still change — L42 recurred as a snapshot count re-derived from a growing log | 2026-09-23 | ~885
...
L60 | tooling  | style-gates,whole-repo-scope,config-globs,presence-vs-closure,lesson-recurrence                | A gitignored path is still in markdownlint-cli2's reach — every tool that writes markdown into the checkout reddens a local lint:md, and one ignores entry per writer recurs | 2026-09-25 | ~890
```
