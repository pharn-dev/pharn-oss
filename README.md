...
For a small, well-scoped change, `--quick` (6.24.0) runs a shorter spine — both human gates, a
...
| `/pharn-ship`           | Run the full workflow once, then present the ship record and briefing at the final human decision gate. `--quick` (6.24.0) runs a shorter spine for a small change and skips `/pharn-regress`'s base comparison.                                           |
...
- **Floor checkers — 86** `.mjs` files under `pharn/floor/` (tests excluded).
...
- **It is token-hungry by construction.** `/pharn-grill` runs the grillers over your plan and
  ...
  well-scoped change, `/pharn-ship --quick` (6.24.0) is the manual lever: it drops `/pharn-regress`'s
