...
[![pharn](https://img.shields.io/badge/pharn-6.5.2-blue)](./CHANGELOG.md)
...
- **Contracts — 10** (`pharn/pharn-contracts/`): `cost-ledger`, `eval-format`, `finding-shape`, `loop-record`, `reconciliation-record`, `regression-report`, `seam-config`, `ship-briefing`, `ship-record`, `verify-report`.
...
- **Floor checkers — 59** `.mjs` files under `pharn/floor/` (tests excluded).
