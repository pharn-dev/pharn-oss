...
| memory poisoning        | promotion to canon is a **gated write** with per-entry provenance                                                           | pre-write hook — `Write`/`Edit`/`MultiEdit`/`NotebookEdit` only; a `Bash` write reaches canon unhooked and undetected (`LIMITS.md §6`) |
...
7. **`reads`/`writes` declared not enforced** — `writes` is enforced by the pre-write hook;
   ...
   through the Write/Edit/MultiEdit/NotebookEdit tool surface; a `Bash` write is outside the hook and is
   neither denied nor detected, so `writes:` is a declaration again there (residual — `LIMITS.md §6`)._
