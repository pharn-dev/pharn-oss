...
# SPEC with a mapping is RED. A `spec_kind: quick` SPEC (6.24.0, /pharn-ship --quick) is TEMPLATED (0) and reaches
...
# `--mode` (6.24.0): run-start only, m in MARKER_MODES ({"quick"}) — /pharn-ship --quick's one caller. Absent
# writes no `mode` key at all (byte-identical to pre-6.24.0). Read by ship-outcome-core.mjs's runMode(), never
...
# tested non-LLM checkers); `gate2-quick` (6.24.0, /pharn-ship --quick) is FLOOR TOO, over a SMALLER stage
...
# unknown, so no verdict can be bound to the run. APPLICABILITY (6.9.1; forked by MODE in 6.24.0): the stage
...
# `undetermined` — both added at 6.24.0's GATE 2 (review F1: a quick run whose run-start was skipped, after
...
run; `/pharn-ship --quick` (6.24.0) runs a shorter spine over a `spec_kind: quick` mini-SPEC, skips
