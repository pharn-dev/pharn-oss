...
| `spec_kind`         | optional: `test-infra`, `quick` (6.25.0), or `feature` — absent means `feature`. See below     |
...
## The rules (an open form, 6.25.0 — enforced by `pharn/floor/check-spec.mjs` through `pharn/floor/spec-template-core.mjs`)
...
| 9   | (6.25.0) a `quick` SPEC: at most 3 criteria, each verified at `unit` or `integration`           | `quick`            | membership + count        |
