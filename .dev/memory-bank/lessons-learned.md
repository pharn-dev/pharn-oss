...
## L50 — A sweep for a retracted claim must enumerate the cites of the referent that broke, not only the claim's spellings

type: process · concepts: [doc-drift, enumeration, referent-binding, sweep-scope, lesson-recurrence]

**Lesson.** A sweep derived from a claim's own wording reaches only the sites that repeat that wording. When the defect's root cause is a removed or moved referent, the sites that broke are the CITES of it, and their wording is whatever each author wrote. Measured: cites into `README.md` text that no longer exists survived for three different reasons at three different times. `LIMITS.md:146` has cited a README "experiment agenda" since the bootstrap commit, and that text has been gone since the reframe (`a82b073`, 2026-06-24). #166 (2026-08-23) later removed README's attempt-0 mention, orphaning the cites in `pharn/ARCHITECTURE.md`, `THREAT-MODEL.md` and `CLAUDE.md`. And the shipped `/pharn-review` command's cite was written in #197 (2026-09-10), eighteen days after that, with README already lacking the text: dead on arrival. #221 retracted "the one residual" and swept that claim's spellings, correctly by [[L33]]'s recipe. The plan for `drift-audit-6-4-1` did the same and stated its coverage boundary, as [[L49]] asks. Two sites still survived, `LIMITS.md:146` and `pharn-review.md:210-212`, whose wording differs ("experiment agenda"; "the one capability … exists to measure"), and were found only when the grill searched a different anchor phrase. Remedy: when the root cause is a deleted or moved referent, sweep by REFERENT first, meaning every cite of the file or section that went away, on every surface, each classified, and then by claim; declare both axes in the sweep's declaration; add a wrap-tolerant pass, because a phrase split across a line wrap is invisible to a line grep; and DEREFERENCE every cite in a sentence you write or re-author (open the target and confirm it says what the cite claims), since a cite can be dead on arrival and no history of the referent will show it.

**Why it matters.** [[L33]], [[L36]], [[L47]] and [[L49]] each assume the sweep's axis is the claim: L33 says a repair pass misses variant spellings, L36 that a presence set is not a closed set, L47 that repairing by a new count rebuilds the defect, and L49 that a completed-sweep claim is verified only where a checker exists and should state its coverage boundary. Here the boundary was stated and was exceeded anyway, because the sweep's axis (the claim) was not the defect's axis (an orphaned referent); stating a boundary is honest and does not by itself reduce the misses. Nothing reads shipped prose, so a dead cite surfaces only when a reader follows it into nothing. **Bound (P0), and the remedy's status per [[L46]]:** the remedy is discipline. A check would need a maintained registry of removed referents (the [[L29]]/[[L36]] cost) and was deliberately not built, deferred at the maintainer's explicit choice in that increment, so this entry is a PENDING-remedy lesson and says so. **Honest trigger (P7):** one observed instance of this specific axis, inside a family already promoted several times ([[L33]], [[L36]], [[L47]], [[L49]]); by [[L20]]'s bar that is arguably a second occurrence of the enumeration-scope family and not yet of this axis.

**Provenance.**

- feature: `drift-audit-6-4-1`
...
## L62 — A value quoted into a refusal reason must go through a function that cannot throw — `String(v)` throws on parsed JSON, and a refusal that throws is read as a verdict
...
- promoted: 2026-09-25 via gated `/pharn-dev-memory-promote` (human-approved). Accepted as L61; renumbered L62 when merging `origin/main`, where PR #275 had promoted a different L61 first. Re-checked with `check-provenance.mjs` as L62 against the merged canon.

## L63 — A change to how a recorded value is DERIVED can move it into the still-growing part of its referent — L58 recurred through a checker the change left untouched

type: floor · concepts: [temporal-state, referent-binding, append-only, derivation-change, lesson-recurrence]

**Lesson.** Binding a recorded value to its live referent ([[L43]]), and splitting that referent into the part the record could see and the part still being written ([[L58]]), are questions about a VALUE. A change to how the value is DERIVED can move it from one part to the other while every existing comparison of it stays as it was. `cost-dedup-completed-usage` (6.22.1) moved a cost-ledger row's usage from its request's FIRST transcript line, which never changes once written, to the request's line with the MOST output tokens, which keeps growing while the request is still being written. The plan cited L58 and applied it to run membership, which it keyed on the first line on purpose. It listed `check-cost-ledger.mjs` as not touched, and that checker's `--verify-transcript` still compared totals EXACTLY. So a correct ledger emitted while one of its requests was still streaming went RED once the request completed. Remedy: when a change alters how a recorded value is derived, enumerate EVERY existing re-derivation of that value, including checkers the change leaves untouched, and ask L58 of each: which part of the value can still change after the record is made? Compare the fixed part exactly, and bound the growing part.

**Measured, in `cost-dedup-completed-usage`.** An independent review reproduced it (REVIEW R1). At HEAD the ledger verified GREEN before and after the request completed. With the change, it was GREEN at emission and RED afterwards. The write pattern that reaches it was measured, not supposed: on 2,833 of 45,362 local requests, a reply to one of the request's own lines was written before the request's largest line. So a tool can run while the request that invoked it is still growing, and that tool may be the run-stop marker or the emitter itself. No false RED on a real ledger had been observed. The remedy chosen at the ship gate for this increment bounds the growing classes per row. The input and cache classes must match exactly. For `output` and `output_thinking`, recorded above live is RED, and recorded below live is a WARN that names the two causes it cannot tell apart: a request in flight at emission, and a ledger written before 6.22.1.

**Why it matters.** [[L58]] was promoted when [[L42]]'s class recurred through [[L43]]'s remedy. Here it recurred again, in an increment that cited L58 and applied it, but only to the comparison its author had in front of them. That is [[L52]]'s shape (a remedy quantified over a set, discharged for one member) applied to L58: the set is every comparison of the re-derived value, and a checker the plan does not touch is the member nobody looks at. A rule change is exactly when this happens, because it re-bases a value that a stable checker keeps comparing the old way. **Bound (P0):** the remedy is plan-time discipline, not a floor check. Whether a source is still being written is a judgment about the source, not a shape a membership test can see. **Honest trigger (P7):** an independent review found it before merge. The mechanism was reproduced and the platform behaviour that reaches it was measured, but no false RED was observed on a shipped ledger. By [[L20]]'s bar that is a reproduction, not a second observed failure. It was promoted at the human's choice at the ship-stage lesson gate, as [[L36]], [[L47]] and [[L60]] were.
