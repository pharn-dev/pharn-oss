...
- **S1 (important, patch:266)** — `scanRuns()` read `ENOTDIR` (a _file_ planted at a run-state path) the same
...
- stage model: opus — set by the maintainer's instruction, overriding pharn.config.json's sonnet for
  build/regress/verify; routed via Agent subagent; effort not routed. A sonnet stage started the fixes and
  handed off at `fd1c387`; this pass reviewed its `handoff/` work, finished the rest, and ran the runner once.
- **Where the build departs from the list above, and why — each measured before it was written:**
  - **B1 is not a straight port.** The handed-off port copied `protect-trusted-paths.cjs`'s
    `resolveWriteTarget`, which reads `\` as a separator on every platform. On macOS/Linux a backslash is a
    file-name character, so `pharn/features/a\b/../../floor/new.mjs` was judged as `pharn/features/floor/…`
    ...
    therefore toward deny.
  - **A new rule, from the same measurement.** The reserved-path fold (`toKey()`, the ✧-pinned copy) reads
    `\` as `/` and collapses `..`, so `.claude/commands/x\..\..\..\src\y.md` — one file INSIDE
    `.claude/commands/` — folded to `src/y.md` and was allowed. In the permissive posture only, a path
    containing a backslash (payload or resolved target) is now denied, with its own message variant of the
    `reserved` body. The allow-list postures need no such rule: their globs match the unfolded path.
  - **S1, the `<name>` level kept lenient.** The handoff also counted `ENOTDIR` under a state directory's
    `<name>` entry as a scan error; REVIEW.md said to keep that level as it was, because a stray file (a
    `.DS_Store`) would then hold the tree fail-closed with no ceiling. As built: the STATE-DIRECTORY level is
    `lstat`ed and anything present that is not a directory is a scan error; a non-directory `<name>` entry is
    skipped. A file planted at `.pharn/pharn-review/<name>` is handled command-side: `--open` exits 2 there
    and both commands STOP.
  - **Smaller, each pinned by a test:** the project root itself is never an allowed out-of-project write
    (it was, under the quirk the build recorded); the memory rule needs a path strictly inside
    `memory/`; both allowed roots are resolved exactly as a write target is, so an absent root and a target
    under it agree on every symlinked prefix; `readScopeFileState()` refuses to read anything but a regular
    file, which ends the FIFO hang REVIEW.md recorded as pre-existing; the scan-error sentence (minor 2) names
    each state directory it could not read; the in-repo stale bullet has its install variant.
- **The handed-off `uncaughtException` backstop, reviewed and kept.** It only ever turns an exit-1 crash into
  exit 2: an allow is a plain `process.exit(0)` with nothing written to stdout, so it cannot throw into the
  handler, and a deny already exits 2. Demonstrated both ways by the suite (a forced `stdout.write` throw
  denies; the same preload leaves an allow at exit 0).
- **Minor 9** is discharged by the re-run `/pharn-dev-verify`, which re-renders `VERIFY.md`.
- **The fix's own named residuals:** the dual resolution judges a hard link by its own name (creating one
  needs Bash); `CLAUDE_CONFIG_DIR`, `HOME` and `TMPDIR` set the two roots, so an environment pointing one at
  a broad directory widens it (a test pins the `TMPDIR` case); and `protect-trusted-paths.cjs` still reads
  `\` as a separator in its segment walk. Its second check, on the `path.resolve()` target, denied every
  backslash case measured here, and this hook's own two resolutions judge such a path correctly in every
