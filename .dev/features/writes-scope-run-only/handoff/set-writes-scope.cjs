...
// 6.23.0 that default is no longer always the fail-closed DEFAULT_SAFE_SET: in a dev checkout, an
// unsignalled tree, or an installed project with a PHARN run open it is (unchanged); in an INSTALLED
// project with NO run open it is instead the newer, more permissive default (deny PHARN's own installed
// surface only — see `enforce-writes-scope.cjs`'s header and `CLAUDE.md`, "Writes-scope"). This command
