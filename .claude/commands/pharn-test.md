...
- **interactive** (no `--unattended`): ASK — _"This project has no `<level>` test runner, or no per-test results for
  it. Run a test-setup increment first via `/pharn-ship` or `/pharn-loop`?"_ — and **stop this feature's run
  either way**. Never continue to the build, and never start that setup run yourself.
